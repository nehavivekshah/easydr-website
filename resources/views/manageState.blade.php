@extends('layout')
@section('title', 'Manage State - Easy Doctor')
                ...
                <div class="col-12">
                    <div class="wizard-card">
                        <div class="wizard-header">
                            <a href="/admin/states" class="back-btn" title="Back"><i class="bx bx-arrow-back"></i></a>
                            <div class="wizard-header-icon"><i class="bx bx-map"></i></div>
                            <div>
                                <p class="wizard-header-title">{{ $isEdit ? 'Edit State' : 'Add New State' }}</p>
                                <p class="wizard-header-sub">{{ $isEdit ? 'Update state details' : 'Add a state/province linked to a country' }}</p>
                            </div>
                        </div>
                        <form action="{{ route('manageState') }}" method="POST">
                            @csrf
                            ...
                            <div class="wizard-body">
                                        ...
                                        <select name="country" class="form-control" required>
                                            ...
                                            @foreach($countries as $country)
                                                <option value="{{ $country->name ?? '' }}"
                                                    @if(($country->name ?? '') == ($states->country ?? '')) selected @endif>
                                                    {{ $country->name ?? '' }}
                                                </option>
                                            @endforeach
                                        </select>
                                ...
                                <div>
                                    <label class="form-label-styled">State Name <span class="text-danger">*</span></label>
                                    <div class="input-group input-group-styled">
                                        <span class="input-group-text"><i class="bx bx-map"></i></span>
                                        <input type="text" name="name" class="form-control"
                                            placeholder="e.g. Maharashtra, California, Ontario"
                                            value="{{ $states->name ?? '' }}" required />
                                    </div>
                                </div>
                            </div>
                            <div class="wizard-footer">
                                <button type="reset" class="btn-reset"><i class="bx bx-reset me-1"></i> Reset</button>
                                <button type="submit" id="submitButton" class="btn-submit"><i class="bx bx-save me-1"></i>{{ $isEdit ? 'Update State' : 'Save State' }}</button>
                            </div>
                        </form>
                    </div>
                </div>
