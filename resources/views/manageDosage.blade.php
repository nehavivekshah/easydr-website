                ...
                <div class="col-12">
                    ...
                    <div class="wizard-card">
                        ...
                        <div class="wizard-header">
                            <a href="/admin/dosages" class="back-btn" title="Back to Dosages">
                            ...
                            <div>
                                <p class="wizard-header-title">{{ $isEdit ? 'Edit Dosage' : 'Add New Dosage' }}</p>
                                <p class="wizard-header-sub">{{ $isEdit ? 'Update the dosage details below' : 'Enter the dosage name to add it to the system' }}</p>
                            </div>
                        </div>

                        {{-- Form --}}
                        <form action="{{ route('manageDosage') }}" method="POST">
                            @csrf
                            <input type="hidden" name="id" value="{{ $_GET['id'] ?? '' }}" />

                            <div class="wizard-body">
                                <label class="form-label-styled">
                                    Dosage Name <span class="text-danger">*</span>
                                </label>
                                <div class="input-group input-group-styled">
                                    ...
                                    <input type="text" name="name" class="form-control"
                                        ...
                                        value="{{ $dosage->name ?? '' }}" required />
                                </div>
                                <div class="mt-2" style="font-size:.76rem;color:#64748b;">
                                    <i class="bx bx-info-circle me-1"></i>
                                    Specify the exact dosage amount and form, e.g. <em>500mg</em>, <em>1 tablet</em>, <em>5ml syrup</em>.
                                </div>
                            </div>
                            ...
                            <div class="wizard-footer">
                                ...
                                <button type="submit" id="submitButton" class="btn-submit">
                                    ...
                                    {{ $isEdit ? 'Update Dosage' : 'Save Dosage' }}
                                </button>
                            </div>
                        </form>
                    </div>
