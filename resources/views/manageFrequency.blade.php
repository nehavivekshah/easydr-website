@extends('layout')
@section('title', 'Manage Frequency - Easy Doctor')
...
<style>
    ...
    .btn-reset  { background:#f1f5f9; color:#374151; border:1.5px solid #e2e8f0; border-radius:50px; padding:10px 22px; font-weight:600; font-size:.88rem; transition:all .2s; cursor:pointer; }
                ...
                <div class="col-12">
                    ...
                    <div class="wizard-card">
                        ...
                        <div class="wizard-header">
                            <a href="/admin/frequencies" class="back-btn" title="Back to Frequencies">
                            ...
                            <div>
                                <p class="wizard-header-title">{{ $isEdit ? 'Edit Frequency' : 'Add New Frequency' }}</p>
                                <p class="wizard-header-sub">{{ $isEdit ? 'Update the frequency details below' : 'Enter a dosage frequency to add to the system' }}</p>
                            </div>
                        </div>

                        {{-- Form --}}
                        <form action="{{ route('manageFrequency') }}" method="POST">
                            @csrf
                            <input type="hidden" name="id" value="{{ $_GET['id'] ?? '' }}" />

                            <div class="wizard-body">
                                <label class="form-label-styled">
                                    Frequency Name <span class="text-danger">*</span>
                                </label>
                                <div class="input-group input-group-styled">
                                    <span class="input-group-text"><i class="bx bx-time-five"></i></span>
                                    <input type="text" name="name" class="form-control"
                                        placeholder="e.g. Once daily, Twice a day, Every 8 hours"
                                        value="{{ $frequency->name ?? '' }}" required />
                                </div>
                                <div class="mt-2" style="font-size:.76rem;color:#64748b;">
                                    <i class="bx bx-info-circle me-1"></i>
                                    Describe how often the medication should be taken, e.g. <em>Once daily</em>, <em>Twice a day</em>, <em>Every 8 hours</em>.
                                </div>
                            </div>
                            ...
                            <div class="wizard-footer">
                                ...
                                <button type="submit" id="submitButton" class="btn-submit">
                                    ...
                                    {{ $isEdit ? 'Update Frequency' : 'Save Frequency' }}
                                </button>
                            </div>
                        </form>
                    </div>
