            ...
            <div class="row">
                <div class="col-lg-3 col-md-4 col-sm-4 offset-md-4 rounded bg-white shadow-sm border mt-4 p-0">
                    <div class="text py-3 px-3 border border-left-0 border-right-0 border-top-0">
                        <h6 class="m-0">
                            <a href="/admin/frequencies" class="text-dark" title="Back">
                                <i class="bx bx-arrow-back h6"></i>
                            </a>  
                            <label class="px-3">
                                @if(!empty($_GET['id'])) {{ 'Edit Frequency' }} @else {{ 'Add New Frequency' }} @endif
                            </label>
                        </h6>
                    </div>

                    <form action="{{ route('manageFrequency') }}" method="POST" class="card-body py-3 px-3">
                        @csrf
                        <div class="form-group">
                            <label class="small">Frequency Name*</label><br />
                            <div class="input-group">
                                <img src="{{ asset('/public/assets/icons/user.svg') }}" class="input-icon" />
                                <input type="text" name="name" class="form-control" placeholder="e.g. Once daily, Twice a day" value="{{ $frequency->name ?? '' }}" required />
                            </div>
