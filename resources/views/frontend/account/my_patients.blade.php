...
@section('content')
    <style>
        ...
        /* Reuse standardized soft badges but keep specific ones if needed */
        .p-badge {
            padding: 4px 12px;
            border-radius: 8px;
            font-size: 0.75rem;
                    ...
                    <div class="col-lg-9">
                        <div class="dashboard_content">
                            <div class="d-flex justify-content-between align-items-center mb-4">
                                ...
                                <div class="badge rounded-pill px-3 py-2">{{ count($patients) }} Total Patients
                                </div>
                            </div>
                    ...
                    <div class="tab-pane fade modal-body" id="tab-refer">
                        <form id="referralForm">
                            <div class="mb-3">
                                ...
                                <select class="form-select w-100 mb-3 rounded-3 p-3" id="refer-specialist">
                            ...
                            <div class="mb-3">
                                ...
                                <select class="form-select w-100 mb-3 rounded-3 p-3" id="refer-doctor">
                    ...
                    <div class="tab-pane fade modal-body" id="tab-add-prescription">
                        <form id="prescriptionForm" class="row g-3">
                            ...
                            <div class="col-12 position-relative">
                                ...
                                <input type="text" class="form-control rounded-3 p-3 mb-3" id="presc-medicine"
                                    autocomplete="off" required placeholder="Search for medicine (e.g. Paracetamol)...">
                            ...
                            <div class="col-md-6">
                                ...
                                <select class="form-select w-100 mb-3 rounded-3 p-3" id="presc-dosage" required>
                                    <option value="">Select Dosage</option>
                                </select>
                            </div>
                            <div class="col-md-6">
                                <label class="form-label fw-bold small text-uppercase">Frequency</label>
                                <select class="form-select w-100 mb-3 rounded-3 p-3" id="presc-frequency" required>
                                    <option value="">Select Frequency</option>
                                </select>
                            </div>
                            <div class="col-md-6">
                                <label class="form-label fw-bold small text-uppercase">Duration</label>
                                <select class="form-select w-100 mb-3 rounded-3 p-3" id="presc-duration" required>
                                    <option value="">Select Duration</option>
                                </select>
                            </div>
                            <div class="col-md-6">
                                <label class="form-label fw-bold small text-uppercase">Route</label>
                                <select class="form-select w-100 mb-3 rounded-3 p-3" id="presc-route">
                                    <option value="">Select Route</option>
                                </select>
                            </div>
                            <div class="col-md-12">
                                <label class="form-label fw-bold small text-uppercase">Food Intake (Meal)</label>
                                <select class="form-select w-100 mb-3 rounded-3 p-3" id="presc-meal">
    ...
    @push('scripts')
        <script>
            ...
            // URL Parameter detection for deep-linking
            window.addEventListener('load', () => {
                const params = new URLSearchParams(window.location.search);
                const pid = params.get('patient_id');
                const eid = params.get('edit_prescription');

                if (pid) {
                    const patient = patientData.find(p => p.id == pid);
                    if (patient) {
                        openPatientDetails(patient.id, patient.first_name + ' ' + patient.last_name, patient.patient_table_id);

                        if (eid) {
                            // If we have an edit ID, switch to the prescriptions tab after a small delay
                            setTimeout(() => {
                                const prescTab = document.querySelector('a[href="#tab-prescriptions"]');
                                if (prescTab) {
                                    bootstrap.Tab.getOrCreateInstance(prescTab).show();
                                }
                            }, 600);
                        }
                    }
