@extends('layout')
@section('title', 'Manage Duration - Easy Doctor')
                ...
                <div class="col-12">
                    <div class="wizard-card">
                        <div class="wizard-header">
                            <a href="/admin/durations" class="back-btn" title="Back"><i class="bx bx-arrow-back"></i></a>
                            <div class="wizard-header-icon"><i class="bx bx-hourglass"></i></div>
                            <div>
                                <p class="wizard-header-title">{{ $isEdit ? 'Edit Duration' : 'Add New Duration' }}</p>
                                <p class="wizard-header-sub">{{ $isEdit ? 'Update the duration details' : 'Enter a treatment duration period' }}</p>
                            </div>
                        </div>
                        <form action="{{ route('manageDuration') }}" method="POST">
                            @csrf
                            <input type="hidden" name="id" value="{{ $_GET['id'] ?? '' }}" />
                            <div class="wizard-body">
                                <label class="form-label-styled">Duration <span class="text-danger">*</span></label>
                                <div class="input-group input-group-styled">
                                    ...
                                    <input type="text" name="name" class="form-control"
                                        ...
                                        value="{{ $duration->name ?? '' }}" required />
                                </div>
                                <div class="mt-2" style="font-size:.76rem;color:#64748b;"><i class="bx bx-info-circle me-1"></i>e.g. <em>3 days</em>, <em>1 week</em>, <em>1 month</em>.</div>
                            </div>
                            <div class="wizard-footer">
                                <button type="reset" class="btn-reset"><i class="bx bx-reset me-1"></i> Reset</button>
                                <button type="submit" id="submitButton" class="btn-submit"><i class="bx bx-save me-1"></i>{{ $isEdit ? 'Update Duration' : 'Save Duration' }}</button>
                            </div>
                        </form>
                    </div>
                </div>
