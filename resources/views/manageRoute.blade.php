@extends('layout')
@section('title', 'Manage Medicine Route - Easy Doctor')
                ...
                <div class="col-12">
                    <div class="wizard-card">
                        <div class="wizard-header">
                            <a href="/admin/routes" class="back-btn" title="Back"><i class="bx bx-arrow-back"></i></a>
                            <div class="wizard-header-icon"><i class="bx bx-transfer-alt"></i></div>
                            <div>
                                <p class="wizard-header-title">{{ $isEdit ? 'Edit Medicine Route' : 'Add New Medicine Route' }}</p>
                                <p class="wizard-header-sub">{{ $isEdit ? 'Update the route details' : 'Enter the administration route for a medicine' }}</p>
                            </div>
                        </div>
                        <form action="{{ route('manageRoute') }}" method="POST">
                            @csrf
                            <input type="hidden" name="id" value="{{ $_GET['id'] ?? '' }}" />
                            <div class="wizard-body">
                                <label class="form-label-styled">Route Name <span class="text-danger">*</span></label>
                                <div class="input-group input-group-styled">
                                    <span class="input-group-text"><i class="bx bx-transfer-alt"></i></span>
                                    <input type="text" name="name" class="form-control"
                                        placeholder="e.g. Oral, IV, Topical, Inhalation"
                                        value="{{ $route->name ?? '' }}" required />
                                </div>
                                <div class="mt-2" style="font-size:.76rem;color:#64748b;"><i class="bx bx-info-circle me-1"></i>e.g. <em>Oral</em>, <em>Intravenous (IV)</em>, <em>Topical</em>, <em>Inhalation</em>.</div>
                            </div>
                            <div class="wizard-footer">
                                <button type="reset" class="btn-reset"><i class="bx bx-reset me-1"></i> Reset</button>
                                <button type="submit" id="submitButton" class="btn-submit"><i class="bx bx-save me-1"></i>{{ $isEdit ? 'Update Route' : 'Save Route' }}</button>
                            </div>
                        </form>
                    </div>
                </div>
