...
<section class="task__section">
    <div class="text">
        <a href="/admin/upcoming-appointments" class="btn btn-default btn-sm back-btn"><i class="bx bx-arrow-back"></i></a>
        {{ $pagename ?? 'Book an Appointment' }}
    </div>
    <div class="container-fluid">
        <div class="row">
            <div class="col-md-12">
                <form action="/admin/manage-appointment" method="post" class="row g-3 px-2" enctype="multipart/form-data">
                    @csrf
                    <div class="col-md-12 bg-white rounded border shadow-sm py-3 px-4">
                                ...
                                <div class="input-group">
                                    ...
                                    <select class="form-control" id="patient_id" name="patient_id" required>
                                        ...
                                        @foreach($patients as $patient)
                                            <option value="{{ $patient->id }}" @if(($appointments->pid ?? '') == ($patient->id ?? '')) selected @endif>{{ $patient->first_name.' '.$patient->last_name }}</option>
                                        @endforeach
                                    </select>
                                ...
                                <div class="input-group">
                                    ...
                                    <select class="form-control" id="doctor_id" name="doctor_id" required>
                                        ...
                                        @foreach($doctors as $doctor)
                                            <option value="{{ $doctor->id }}" @if(($appointments->did ?? '') == ($doctor->id ?? '')) selected @endif>{{ $doctor->first_name.' '.$doctor->last_name }}</option>
                                        @endforeach
                                    </select>
                                </div>
                            </div>
                            ...
                            <div class="form-group col-md-6">
                                ...
                                <div class="input-group">
                                    ...
                                    <select class="form-control" id="appointment_date" name="appointment_date" required>
                                        ...
                                        <option class="{{$appointments->date ?? ''}}" selected>{{ $appointments->date ?? '' }}</option>
                                        @endif
                                    </select>
                                </div>
                            </div>
                            ...
                            <div class="form-group col-md-6">
                                ...
                                <div class="input-group">
                                    ...
                                    <select class="form-control" id="appointment_time" name="appointment_time" required>
                                        ...
                                        <option class="{{date_format(date_create($appointments->time ?? null), 'H:i')}}" selected>{{date_format(date_create($appointments->time ?? null), 'h:i A')}}</option>
                                        @endif
                                    </select>
                                </div>
                            </div>
                        ...
                        <div class="form-group col-md-12">
                            ...
                            <textarea class="form-control border" id="problems" name="problems" rows="3" required placeholder="Describe the patient's problems...">{{ $appointments->note ?? '' }}</textarea>
                        </div>
