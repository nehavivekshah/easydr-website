                ...
                <div class="col-lg-8 col-md-10 rounded bg-white shadow-sm border mt-4 p-0">
                    <div class="text py-3 px-3 border border-left-0 border-right-0 border-top-0">
                        <h6 class="m-0">
                            ...
                            <label class="px-3">@if(request('id')) {{ 'Edit Medicine' }} @else {{ 'Add New Medicine' }}
                            @endif</label>
                        </h6>
                    </div>

                    <form action="{{ route('manageMedicine') }}" method="POST" enctype="multipart/form-data"
                        ...
                        <div class="row">
                                ...
                                <div class="input-group d-flex gap-2 align-items-center">
                                    ...
                                    @if(!empty($gallery))
                                        @foreach(array_slice($gallery, 0, 4) as $img)
                                            <img src="{{ asset('/public/' . $img) }}" class="thumbnail rounded-start"
                            ...
                            <div class="form-group col-md-6">
                                ...
                                <div class="input-group mb-2">
                                    ...
                                    <input type="number" step="0.01" name="cost" class="form-control"
                                        placeholder="Original Cost" value="{{ $medicine->cost ?? 0 }}" required="">
                                </div>
                            </div>
                            ...
                            <div class="form-group col-md-6">
                                ...
                                <div class="input-group">
                                    ...
                                    <input type="number" step="0.01" name="discount_cost" class="form-control"
                                        placeholder="Discounted Cost" value="{{ $medicine->discount_cost ?? 0 }}">
                                </div>
                            </div>
                                ...
                                <div class="input-group">
                                    ...
                                    <select name="type_id" class="form-select" required>
                                        @foreach($types as $type)
                                            <option value="{{ $type->id }}" @if(($type->id ?? '') == ($medicine->type_id ?? ''))
                                            selected @endif>
                                                {{ $type->name }}
                                            </option>
                                        @endforeach
                            ...
                            <div class="form-group col-md-6">
                                ...
                                <div class="input-group">
                                    ...
                                    <select name="medicine_category" class="form-select" required="">
                                        ...
                                        <option value="otc" @if(($medicine->medicine_category ?? '') == 'otc') selected
                                        @endif>OTC</option>
                                        <option value="prescribed" @if(($medicine->medicine_category ?? '') == 'prescribed')
                                        selected @endif>Prescribed</option>
                                        <option value="prescribed" @if(($medicine->medicine_category ?? '') == 'spacial')
                                        selected @endif>Spacial Medicine</option>
                                    </select>
                                </div>
                            </div>
                                ...
                                <div class="input-group">
                                    ...
                                    <select name="store_id" id="pharmacySelect" class="form-select" required>
                                        ...
                                        @foreach($stores as $store)
                                            <option value="{{ $store->PharmacyID }}" @if(($store->PharmacyID ?? '') == ($medicine->pharmacy_id ?? '')) selected @endif>{{ $store->PharmacyName }}
                                            </option>
                                        @endforeach
                                    </select>
                                </div>
                            ...
                            <div class="form-group col-md-6">
                                ...
                                <div class="input-group">
                                    ...
                                    <select name="store_locations[]" id="storeLocationsSelect" class="form-control"
                                        multiple="multiple" required>
                                        @foreach($locations as $location)
                                            <option value="{{ $location->LocationID  }}" @if(($medicine->store_id ?? '') == ($location->LocationID ?? '')) selected @endif>
                            ...
                            <div class="form-group col-md-6">
                                ...
                                <div class="input-group">
                                    ...
                                    <input type="text" name="medical_stream" class="form-control"
                                        placeholder="Enter Medical Stream" value="{{ $medicine->medical_stream ?? '' }}"
                                        required>
                                </div>
                            </div>
                            ...
                            <div class="form-group col-md-6">
                                ...
                                <textarea name="symptoms" class="form-control border" rows="3"
                                    placeholder="Enter Symptoms">{{ $medicine->symptoms ?? '' }}</textarea>
                            </div>

                            <div class="form-group col-md-6">
                                <label class="small">Description*</label><br />
                                <textarea name="description" class="form-control border" rows="3"
                                    placeholder="Enter Description">{{ $medicine->description ?? '' }}</textarea>
                            </div>
                            ...
                            <div class="form-group col-md-6">
                                ...
                                <div class="input-group">
                                    ...
                                    <select name="available" class="form-select" required>
                                        <option value="1" {{ ($medicine->available ?? '') == 1 ? 'selected' : '' }}>Yes
                                        </option>
                                        <option value="0" {{ ($medicine->available ?? '') == 0 ? 'selected' : '' }}>No
                                        </option>
                                    </select>
                                </div>
                            </div>
