                ...
                <div class="col-12">
                    <div class="wizard-card">
                        <div class="wizard-header">
                            <a href="/admin/cities" class="back-btn" title="Back"><i class="bx bx-arrow-back"></i></a>
                            <div class="wizard-header-icon"><i class="bx bx-buildings"></i></div>
                            <div>
                                <p class="wizard-header-title">{{ $isEdit ? 'Edit City' : 'Add New City' }}</p>
                                <p class="wizard-header-sub">{{ $isEdit ? 'Update city details' : 'Add a city linked to a country and state' }}</p>
                            </div>
                        </div>
                        <form action="{{ route('manageCity') }}" method="POST">
                            @csrf
                            ...
                            <div class="wizard-body">
                                ...
                                {{-- Country --}}
                                <div class="mb-4">
                                    <label class="form-label-styled">Country <span class="text-danger">*</span></label>
                                        ...
                                        <select name="country" id="country" class="form-control" required>
                                            ...
                                            @foreach($countries as $country)
                                                <option value="{{ $country->name ?? '' }}"
                                                    @if(($country->name ?? '') == ($cities->country ?? '')) selected @endif>
                                                    {{ $country->name ?? '' }}
                                                </option>
                                            @endforeach
                                        </select>
                                ...
                                {{-- State --}}
                                <div class="mb-4">
                                    <label class="form-label-styled">State <span class="text-danger">*</span></label>
                                    <div class="input-group input-group-styled">
                                        <span class="input-group-text"><i class="bx bx-map"></i></span>
                                        <select name="state" id="state" class="form-control" required>
                                            @foreach($states as $state)
                                                <option value="{{ $state->name ?? '' }}"
                                                    @if(($state->name ?? '') == ($cities->state ?? '')) selected @endif>
                                                    {{ $state->name ?? '' }}
                                                </option>
                                            @endforeach
                                        </select>
                                ...
                                {{-- City Name --}}
                                <div>
                                    <label class="form-label-styled">City Name <span class="text-danger">*</span></label>
                                    <div class="input-group input-group-styled">
                                        <span class="input-group-text"><i class="bx bx-buildings"></i></span>
                                        <input type="text" name="name" class="form-control"
                                            placeholder="e.g. Mumbai, New York, London"
                                            value="{{ $cities->name ?? '' }}" required />
                                    </div>
                                </div>
                            ...
                            <div class="wizard-footer">
                                ...
                                <button type="submit" id="submitButton" class="btn-submit"><i class="bx bx-save me-1"></i>{{ $isEdit ? 'Update City' : 'Save City' }}</button>
                            </div>
                        </form>
                    </div>
                </div>
