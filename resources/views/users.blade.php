...
@section('content')
    ...
    <section class="task__section">
        <div class="text">
            ...
            @if(in_array('users_add',$roleArray) || in_array('patients_add',$roleArray) || in_array('doctors_add',$roleArray) 
                || in_array('admin_accounts_add',$roleArray) || in_array('staff_accounts_add',$roleArray) || in_array('All',$roleArray))
            <div class="btn-group">
                <a href="/admin/manage-user/{{$type}}" class="btn btn-default btn-sm"><i class="bx bx-plus"></i> <span>Add New</span></a>
            </div>
            @endif
        </div>
                    ...
                    <table id="lists" class="table table-striped table-bordered m-table" style="width:100%">
                        ...
                        <tbody>
                            ...
                            <tr>
                                ...
                                <td class="m-none"><img src="/public/assets/images/profiles/{{$user->photo ?? '--'}}" class="media-icon" /></td>
                                ...
                                @endif
                                <td class="text-center">@if($user->status == '1')<span class="font-weight-bold badge bg-success">Active</span>@else<span class="font-weight-bold badge bg-danger">Deactive</span>@endif</td>
                                <td class="text-center">
                                    @if(in_array('users_edit',$roleArray) || in_array('patients_edit',$roleArray) || in_array('doctors_edit',$roleArray) 
                                        || in_array('admin_accounts_edit',$roleArray) || in_array('staff_accounts_edit',$roleArray) || in_array('All',$roleArray))
                                    <a href="/admin/manage-user/{{$type}}/{{ $user->id }}" class="btn btn-info btn-sm" title="Edit"><i class="bx bx-edit"></i></a>
                                    @endif
                                    
                                    @if(in_array('users_delete',$roleArray) || in_array('patients_delete',$roleArray) || in_array('doctors_delete',$roleArray) 
                                        || in_array('admin_accounts_delete',$roleArray) || in_array('staff_accounts_delete',$roleArray) || in_array('All',$roleArray))
                                        <!--<form action="/admin/cancel-appointment/{{ $user->id }}" 
                                              method="POST" 
                                              style="display:inline;" 
                                              onsubmit="return confirm('Are you sure you want to cancel this appointment?');">
                                            @csrf
                                            <button type="submit" 
                                                    class="btn {{ ($user->status === 2) ? 'text-muted disabled' : 'btn-danger' }} btn-sm" 
                                                    title="Deactive">
                                                <i class="bx bx-x"></i>
                                            </button>
                                        </form>-->
